...
use crate::{
  ...
  io::{
    case_format::{SpartanLikeCase, load_spartan_like_case_from_dir},
  },
  sumcheck::{
    inner::{
      SumcheckTrace,
      VerifyTrace,
      inner_product,
      prove_inner_sumcheck_with_label,
...
pub fn format_spartan_like_report(data: &SpartanLikeReportData) -> String {
  ...
  for (i, r) in data.r_x.iter().enumerate() {
    ...
    out.push_str(&format!("    w^({}) = {}\n", i + 1, fmt_vec(&data.eq_states[i + 1])));
  }
  ...
  out.push_str(&explain_bound("A", &data.case.a, &data.row_weights, &data.a_bound));
  out.push_str(&explain_bound("B", &data.case.b, &data.row_weights, &data.b_bound));
  out.push_str(&explain_bound("C", &data.case.c, &data.row_weights, &data.c_bound));
