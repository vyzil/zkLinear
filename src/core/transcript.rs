...
use crate::{core::field::Fp, protocol::spec_v1::append_u64_le};

pub fn derive_round_challenge(label: &[u8], round: usize, h0: Fp, h1: Fp, h2: Fp) -> Fp {
    let mut hasher = Sha256::new();
    hasher.update(label);
    hasher.update((round as u64).to_le_bytes());
    hasher.update(h0.0.to_le_bytes());
    hasher.update(h1.0.to_le_bytes());
    hasher.update(h2.0.to_le_bytes());
...
pub fn derive_round_challenge_merlin(
    ...
    tr.append_message(b"msg_0", &h0.0.to_le_bytes());
    tr.append_message(b"msg_1", &h1.0.to_le_bytes());
    tr.append_message(b"msg_2", &h2.0.to_le_bytes());
    let mut out = [0u8; 32];
