...
use self::{
  commit::commit,
  prove::prove_eval,
  types::{BrakedownEvalProof, BrakedownParams, BrakedownProverCommitment, BrakedownVerifierCommitment, MiniEncoding},
  verify::verify_eval,
};
...
pub struct BrakedownPcs {
  ...
  pub encoding: MiniEncoding,
}

impl BrakedownPcs {
  pub fn new(params: BrakedownParams) -> Self {
    let encoding = MiniEncoding::new(params.n_per_row);
