...
fn brakedown_002_spielman_encoding_changes_with_seed() {
    run_instance!(
        ...
        "encoder=SpielmanLike seed=1 vs 2",
        {
...
fn brakedown_003_pcs_open_verify_succeeds_and_wrong_claim_fails() {
    run_instance!(
        ...
        "transcript=PCS_DEMO",
        {
            ...
            testlog::data("open_cols", proof.columns.len());
            testlog::data("degree_tests", proof.p_random_vec.len());
...
fn brakedown_006_wire_roundtrip_succeeds_and_rejects_trailing_bytes() {
    run_instance!(
        ...
        "expect: roundtrip verify succeeds, malformed trailing bytes rejected",
        {
            let mut params = BrakedownParams::new_toy(8);
            params.encoder_kind = BrakedownEncoderKind::SpielmanLike;
            let pcs = BrakedownPcs::new(params);
            ...
            testlog::data("vc_bytes", vc_bytes.len());
            testlog::data("pf_bytes", pf_bytes.len());
